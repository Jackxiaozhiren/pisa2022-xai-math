...
Mathematics literacy is a core indicator of students' readiness to reason quantitatively, solve real-world problems, and participate in knowledge-based societies. PISA 2022 is a timely setting for examining this outcome because mathematics was the major assessment domain and the assessment followed a period of severe disruption to schooling and digital learning arrangements \citep{OECD_2022_results_vol1,OECD_2022_results_vol2}. The public-use files also include student, family, school, and ICT-related questionnaire variables, which makes it possible to examine mathematics performance together with students' learning resources and technology contexts \citep{OECD_2022_database}.

Educational data mining and learning analytics research increasingly uses machine learning to predict achievement and identify students who may need support \citep{Romero_Ventura_2020,Ifenthaler_Yau_2020}. This work is useful because tree-based and ensemble models can capture nonlinear predictive structure that is difficult to represent manually \citep{Breiman_2001,Fernandez_Delgado_2014}. However, prediction studies using large-scale assessment data face two additional obligations. First, PISA estimates should acknowledge plausible values, final student weights, and replicate weights \citep{OECD_2022_technical}. Second, model explanations should not be interpreted as evidence of what would happen if a family, school, or technology condition were changed.
...
PISA 2022 includes indicators related to ICT resources, ICT self-efficacy, ICT availability or use at home and school, digital information behavior, subject-related ICT use, and homework/study indicators \citep{OECD_2022_results_vol2}. These variables are central to an educational technology contribution because they distinguish between access, confidence, learning use, and possible distraction rather than treating "technology" as a single exposure.
...
Permutation importance for the LightGBM classification model identified home possessions (\texttt{HOMEPOS}) as the strongest predictor, followed by mathematics self-efficacy (\texttt{MATHEFF}), family connection (\texttt{FAMCON}), grade, ICT resources (\texttt{ICTRES}), cognitive activation, ICT self-efficacy (\texttt{ICTEFFIC}), student behavior hindering learning (\texttt{STUBEHA}), parental occupational status (\texttt{HISEI}), and mathematics anxiety (\texttt{ANXMAT}). The same broad pattern appeared in the regression model, where \texttt{HOMEPOS} and \texttt{MATHEFF} had the largest importance values by a wide margin.
...
Machine-learning workflows do not perfectly align with complex survey estimation. This study uses student weights in model fitting and holdout metrics where supported, and it reports BRR and plausible-value descriptive estimates, but prediction metrics remain model-evaluation quantities rather than population parameter estimates. Future work could extend this workflow with uncertainty-aware prediction, such as conformal prediction, while retaining the distinction between predictive uncertainty and causal uncertainty \citep{Angelopoulos_Bates_2021}.
...
Tables 1--9 and Figures 1--3 are generated from the reproducible analysis scripts. The source artifact routing is documented in the submission package and can be provided as supplementary or reviewer material together with the aggregate CSV tables and figure files.
